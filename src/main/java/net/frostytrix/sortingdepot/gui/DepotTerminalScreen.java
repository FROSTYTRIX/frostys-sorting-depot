...
public class DepotTerminalScreen extends AbstractContainerScreen<DepotTerminalMenu> {
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        ...
        if (event.button() == 0) {
            ...
            if (event.y() >= guiY + HEADER_Y && event.y() < guiY + HEADER_Y + ROW_HEIGHT
                    && event.x() >= guiX + 8 && event.x() < guiX + 8 + rowWidth()) {
                int third = rowWidth() / 3;
                int rel = (int) event.x() - (guiX + 8);
    ...
    protected void extractLabels(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        ...
        for (SortField field : SortField.values()) {
            graphics.text(this.font, headerLabel(field), columnX(field), HEADER_Y, HEADER_COLOR, false);
        }
